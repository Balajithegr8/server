...
occupancy_data = []
occupancy_save_interval = 3  # Save occupancy data every 60 seconds
...
while(cap.isOpened()):
    ...
    if dict['parking_detection']:
        ...
        new_occupancy_status = [bool(status) for status in parking_status]
    ...
    if dict['parking_detection'] and (video_cur_pos - last_occupancy_save_time) >= occupancy_save_interval:
        with open("occupancy_data_realtime.json", "w") as json_file:
            json.dump(occupancy_data, json_file, indent=2)
        last_occupancy_save_time = video_cur_pos
